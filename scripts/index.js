...
function closePopupKey(popupName) {
  const cleanPopupEscape = (evt) =>{
    if(evt.key ==='Escape'){
      closePopup(popupName);
      evt.target.removeEventListener('keydown', cleanPopupEscape);
    }
  }

  if(popupName.classList.contains('popup_opened')){
    document.addEventListener('keydown', cleanPopupEscape);
  }
}
